...
const UserSettingsModal = ({ isOpen, onClose }) => {
  ...
  const [faceIDEnabled, setFaceIDEnabled] = useState(() => {
    // Dùng cache từ localStorage để hiển thị ngay, tránh nhấp nháy
    try {
      const cached = localStorage.getItem('faceIDEnabled');
      return cached === 'true';
    } catch {
      return false;
    }
  ...
  const checkFaceIDStatus = async () => {
    try {
      ...
      if (user && user.id) {
        ...
        localStorage.setItem('faceIDEnabled', String(hasFace));
      }
    } catch (error) {

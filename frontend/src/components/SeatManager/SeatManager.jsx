...
const SeatManager = ({ onSeatsChange, showtimeId }) => {
  ...
  const deleteSeatHandler = async (seatId) => {
    if (!seatId) {
      console.error('No seat ID provided for deletion');
      return;
    }
    ...
    try {
      ...
      setSeats(updatedSeats);
      onSeatsChange(updatedSeats);
      console.log('Seat deleted successfully:', seatId);
    } catch (error) {
      console.error('Error deleting seat:', error);
      
      if (error.message.includes('Failed to fetch') || error.message.includes('CORS')) {
        console.log('API not available, removing from local state only');
        const updatedSeats = seats.filter(s => s.id !== seatId);
        setSeats(updatedSeats);
        onSeatsChange(updatedSeats);
        alert('Đã xóa ghế khỏi giao diện. Lưu ý: Ghế vẫn còn trong database do lỗi kết nối API.');
      } else {
  ...
  const clearAllSeats = async () => {
    ...
    if (window.confirm('Bạn có chắc chắn muốn xóa tất cả ghế?')) {
      try {
        ...
        setPendingSeats([]);
        onSeatsChange([]);
        console.log('All seats cleared successfully from database');
      } catch (error) {
        console.error('Error clearing all seats from database:', error);
        
        if (error.message.includes('Failed to fetch') || error.message.includes('CORS')) {
          console.log('API not available, clearing local state only');
          setSeats([]);
          setPendingSeats([]);
          onSeatsChange([]);
          alert('Đã xóa ghế khỏi giao diện. Lưu ý: Ghế vẫn còn trong database do lỗi kết nối API.');
    ...
    <div className="seat-manager">
      ...
      <div className="seat-manager-content">
        ...
        <div className="seats-list">
          ...
          <div className="seats-grid-manager">
            {seats.map(seat => (
